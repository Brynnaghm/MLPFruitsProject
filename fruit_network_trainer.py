...
# our parsed input
dropout_rate_value = 0 # include all the data
batch_size = 100

experiment_name = "experiment100_{}_batch_size_{}_bn_{}_mp_{}".format(experiment_prefix,
...
train_data = FruitsDataProvider(which_set="train", batch_size=batch_size, rng=rng)
val_data = FruitsDataProvider(which_set="valid", batch_size=batch_size, rng=rng)
test_data = FruitsDataProvider(which_set="test", batch_size=batch_size, rng=rng)
...
if continue_from_epoch == -1:  # if this is a new experiment and not continuation of a previous one then generate a new
    # statistics file
    save_statistics(logs_filepath, "result_summary_statistics100", ["epoch", "train_c_loss", "train_c_accuracy",
