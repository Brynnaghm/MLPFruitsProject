    ...
    def __call__(self, image_input, training=False, dropout_rate=0.0):
        ...
        with tf.variable_scope(self.name, reuse=self.reuse):
            ...
            with tf.variable_scope('VGGNet'):
                outputs = image_input
                for i in range(len(self.layer_stage_sizes)): # for each size
                    with tf.variable_scope('conv_stage_{}'.format(i)):
                        for j in range(self.inner_layer_depth): # for each layer
                            with tf.variable_scope('conv_{}_{}'.format(i, j)):
                                ...
                                outputs = leaky_relu(outputs, name="leaky_relu{}".format(i)) # make relu
