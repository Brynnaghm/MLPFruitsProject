...
class ClassifierNetworkGraph:
    def __init__(self, input_x, target_placeholder, dropout_rate,
        ...
        layer stage. 3 - as first is the dimensionality reduction?? list [3, 128, 64, 32]
        :param strided_dim_reduction: Whether to use strided convolutions instead of max pooling. false
        """
        ...
        if network_name == "VGG_classifier":
            ...
            # test
            self.c = VGGClassifier(self.batch_size, name="classifier_neural_network",
                                   batch_norm_use=False, num_channels=num_channels,
                                   num_classes=n_classes, layer_stage_sizes=[128, 64, 32],
                                   strided_dim_reduction=False, inner_layer_depth=1) # only do 1 layer of each thing
